    ...
    return calculatePointsWorker.execute(task).then(() => {
      return knex('workload_points_calculations')
        ...
        .then(function (workloadPointsCalculations) {
          ...
          expect(workloadPointsCalculations[0].gs_adjustment_points).to.equal(0)
          expect(workloadPointsCalculations[0].arms_total_cases).to.equal(0)
          expect(workloadPointsCalculations[0].total_points).to.equal(29386)
          expect(workloadPointsCalculations[1].workload_report_id).to.equal(workloadReportId)
          expect(workloadPointsCalculations[1].total_points).to.equal(29377)
          expect(workloadPointsCalculations[2].workload_report_id).to.equal(workloadReportId)
          expect(workloadPointsCalculations[2].total_points).to.equal(29377)
