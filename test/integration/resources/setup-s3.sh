...
aws --endpoint-url=http://localhost:4566 sqs create-queue --queue-name manage-a-sentence-development-hmpps_workload_s3_extract_event_queue
aws --endpoint-url=http://localhost:4566 s3api put-bucket-notification-configuration --bucket wmt-worker --notification-configuration '{"QueueConfigurations":[{"QueueArn":"arn:aws:sqs:eu-west-2:000000000000:manage-a-sentence-development-hmpps_workload_s3_extract_event_queue","Events": ["s3:ObjectCreated:*"]}]}'
