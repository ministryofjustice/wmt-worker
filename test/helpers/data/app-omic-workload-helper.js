...
module.exports.addOmicWorkloadForWorkloadOwner = function (workloadOwnerId) {
  const inserts = []
  return knex('workload_report').withSchema('app').returning('id').insert({})
    .then(function (ids) {
      ...
      return knex('omic_workload').withSchema('app').returning('id').insert(workload)
    }).then(function (ids) {
      inserts.push({ table: 'omic_workload', id: ids[0] })
      return inserts
    })
}

module.exports.getCountOfOmicWorkloadsForWorkloadOwnerId = function (workloadOwnerId) {
  return knex('omic_workload').withSchema('app').count('*').where('workload_owner_id', workloadOwnerId)
