const logger = require('../../app/services/log')

exports.up = function (knex, Promise) {
  return knex.schema.createTable('omic_workload', function (table) {
    table.increments('id')
    table.integer('workload_owner_id').unsigned().references('workload_owner.id')
    table.integer('total_cases').unsigned().notNullable()
    table.integer('total_community_cases').unsigned().notNullable()
    table.integer('total_custody_cases').unsigned().notNullable()
    table.integer('total_license_cases').unsigned().notNullable()
    table.integer('monthly_sdrs').unsigned().notNullable()
    table.integer('sdr_due_next_30_days').unsigned().notNullable()
    table.integer('sdr_conversions_last_30_days').unsigned().notNullable()
    table.integer('paroms_completed_last_30_days').unsigned().notNullable()
    table.integer('paroms_due_next_30_days').unsigned().notNullable()
    table.integer('license_last_16_weeks').unsigned().notNullable()
    table.integer('community_last_16_weeks').unsigned().notNullable()
    table.integer('staging_id').unsigned()
    table.integer('workload_report_id').unsigned()
    table.integer('arms_community_cases').unsigned().notNullable()
    table.integer('arms_license_cases').unsigned().notNullable()
    table.integer('total_t2a_cases').unsigned().notNullable()
    table.integer('total_t2a_community_cases').unsigned().notNullable()
    table.integer('total_t2a_custody_cases').unsigned().notNullable()
    table.integer('total_t2a_license_cases').unsigned().notNullable()
    table.integer('total_filtered_cases').unsigned().notNullable()
    table.integer('total_filtered_community_cases').unsigned().notNullable()
    table.integer('total_filtered_custody_cases').unsigned().notNullable()
    table.integer('total_filtered_license_cases').unsigned().notNullable()
  }).catch(function (error) {
    logger.error(error)
    throw error
  })
}

exports.down = function (knex, Promise) {
  return knex.schema.dropTable('omic_workload')
}
